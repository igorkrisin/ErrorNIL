...
class ProfileTableCell: UITableViewCell {
    ...
    private func setupGesture() {
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(grayCellTapped(_:)))
        grayCellView.addGestureRecognizer(tapGesture)
    }
    
    @objc private func grayCellTapped(_ sender: UITapGestureRecognizer) {
        didTapCell?()
    }
    
    func setupCell(cell: ProfileModelCell, tag: Int) {
        addSubview(mainView)
